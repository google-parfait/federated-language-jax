...
from federated_language_jax.backend import execution_contexts
from federated_language_jax.computation import jax_computation
import numpy as np


class AsyncLocalExecutionContextTest(
    absltest.TestCase, unittest.IsolatedAsyncioTestCase
):

  def test_create_async_local_execution_context_returns_async_context(self):
    context = execution_contexts.create_async_local_execution_context()
    self.assertIsInstance(context, federated_language.framework.AsyncContext)

  @federated_language.framework.with_context(
      execution_contexts.create_async_local_execution_context
  )
...
class SyncLocalExecutionContextTest(absltest.TestCase):

  def test_create_sync_local_execution_context_returns_sync_context(self):
    context = execution_contexts.create_sync_local_execution_context()
    self.assertIsInstance(context, federated_language.framework.SyncContext)

  @federated_language.framework.with_context(
      execution_contexts.create_sync_local_execution_context
  )
