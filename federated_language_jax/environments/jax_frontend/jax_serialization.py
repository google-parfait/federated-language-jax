...
from federated_language_jax.environments.jax_frontend import jax_computation_context
from federated_language_jax.environments.xla_backend import xla_serialization
import jax
import numpy as np
...
class _XlaSerializerStructArg(
    tensorflow_federated.structure.Struct, federated_language.TypedObject
):
  ...
  def __init__(
    ...
    tensorflow_federated.structure.Struct.__init__(self, elements)
  ...
  def __str__(self) -> str:
    return f'_XlaSerializerStructArg({tensorflow_federated.structure.Struct.__str__(self)})'
...
def _struct_flatten(
    struct: tensorflow_federated.structure.Struct,
) -> tuple[tuple[object, ...], tuple[Optional[str], ...]]:
  child_names, child_values = tuple(
      zip(*tensorflow_federated.structure.iter_elements(struct))
  )
...
) -> tensorflow_federated.structure.Struct:
  return tensorflow_federated.structure.Struct(
      tuple(zip(child_names, child_values))
  )


jax.tree_util.register_pytree_node(
    tensorflow_federated.structure.Struct, _struct_flatten, _struct_unflatten
)
