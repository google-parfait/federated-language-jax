...
from federated_language_jax.environments.jax_frontend import jax_serialization
...
class StructPytreeTest(absltest.TestCase):
  ...
  def test_named_struct(self):
    struct = tensorflow_federated.structure.Struct.named(a=1, b=2, c=3)
  ...
  def test_unnamed_struct(self):
    struct = tensorflow_federated.structure.Struct.unnamed(1, 'a', 2)
    children, aux_data = jax.tree_util.tree_flatten(struct)
  ...
  def test_nested_structs(self):
    struct = tensorflow_federated.structure.Struct.named(
        a=1,
        b=tensorflow_federated.structure.Struct.named(
            c=4, d=tensorflow_federated.structure.Struct.unnamed(5, 6), e=7
        ),
    )
    children, aux_data = jax.tree_util.tree_flatten(struct)
  ...
  def test_mixed_nested_structs_and_python_containers(self):
    struct = tensorflow_federated.structure.Struct.named(
        ...
        b=[
            2,
            tensorflow_federated.structure.Struct.named(c=4, d=(5, 6), e=7),
            3,
        ],
    )
    children, aux_data = jax.tree_util.tree_flatten(struct)
